                        ...
                        <div class="col-md-12">
                            ...
                            <div class="d-flex justify-content-between align-items-center flex-wrap breadcrumb-white">
                                <ul class="breadcrumb mb-0">
                                    ...
                                    <li class="breadcrumb-item"><a href="{{ route('duty-roster.index') }}">Duty Roster</a>
                                    </li>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @if (session('message'))
                            <div class="alert alert-{{ session('alert-type', 'info') }} alert-dismissible fade show">
                                <i
                                    class="feather icon-{{ session('alert-type') === 'success' ? 'check-circle' : 'info' }} me-2"></i>
                                {{ session('message') }}
                                                    ...
                                                    <tr class="password-row">
                                                        <td>
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <small
                                                                        class="text-muted">{{ $item['email'] ?? 'No email' }}</small>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <div class="d-flex flex-column">
                                                                <span
                                                                    class="badge bg-{{ $item['arm_of_service'] == 'ARMY' ? 'primary' : ($item['arm_of_service'] == 'NAVY' ? 'info' : 'warning') }} mb-1">
                                                                    {{ $item['arm_of_service'] }}
                                                                </span>
                                                        ...
                                                        <td>
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                <button
                                                                    class="btn btn-sm btn-outline-primary ms-2 toggle-password"
                                                                    data-password="{{ $item['temp_password'] }}"
                                                                    data-target="password-{{ $loop->index }}">
                                                                    <i class="feather icon-eye"></i>
                                                                </button>
                                                                <button
                                                                    class="btn btn-sm btn-outline-success ms-1 copy-password"
                                                                    data-password="{{ $item['temp_password'] }}">
                                                        ...
                                                        <td>
                                                            ...
                                                            @else
                                                                <span class="badge bg-success">
                                                                    ...
                                                                    <br>
                                                                    <small>Expires:
                                                                        {{ $item['expires_at']->diffForHumans() }}</small>
                                                                </span>
                                                            @endif
                                                        </td>
                                                        <td>
                                                            <div class="btn-group">
                                                                @if ($item['is_expired'])
                                                                    <form
                                                                        action="{{ route('dclerk.regenerate-temp-password', $item['user_id']) }}"
                    ...
                    <div class="card mt-4 border-0 shadow-sm">
                        <div class="card-header bg-white text-dark py-3">
                            <h6 class="mb-0"><i class="feather icon-send mr-2 text-primary"></i>Communication Delivery Summary</h6>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered mb-0">
                                    ...
                                    <tbody>
                                        @foreach(session('sms_summary') as $index => $entry)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ $entry['name'] }}</td>
                                                <td>{!! $entry['sms']['status'] !!}</td>
                                                <td>{{ $entry['sms']['message'] }}</td>
                                                <td>{!! $entry['email']['status'] !!}</td>
                                                <td>{{ $entry['email']['message'] }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function copyToClipboard(text) {
                ...
                `;
                    toast.innerHTML =
                        `<i class="feather icon-check-circle me-2"></i> Password copied to clipboard`;
