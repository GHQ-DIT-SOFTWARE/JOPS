...
<section class="pcoded-main-container">
        ...
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('superadmin.users.store') }}" class="forms-sample">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" name="service_no" value="{{ old('service_no') }}" required placeholder="Service Number">
                                <div class="text-danger">{{ $errors->first('service_no') }}</div>
                            </div>

                            <div class="form-group col-md-6">
                                <label>Rank <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="rank" value="{{ old('rank') }}" required placeholder="Rank">
                                <div class="text-danger">{{ $errors->first('rank') }}</div>
                            </div>

                            <div class="form-group col-md-6">
                                <label>Full Name <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" name="fname" value="{{ old('fname') }}" required placeholder="Full Name">
    ...
    <div class="text-danger">{{ $errors->first('unit_id') }}</div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" name="arm_of_service">
                                    ...
                                    <option value="ARMY" {{ old('arm_of_service') == 'ARMY' ? 'selected' : '' }}>ARMY</option>
                                    <option value="NAVY" {{ old('arm_of_service') == 'NAVY' ? 'selected' : '' }}>NAVY</option>
                                    <option value="AIRFORCE" {{ old('arm_of_service') == 'AIRFORCE' ? 'selected' : '' }}>AIRFORCE</option>
                                </select>
    ...
    <select class="form-control" name="is_role" required>
        ...
        <option value="0" {{ old('is_role') == '0' ? 'selected' : '' }}>Super Admin</option>
        <option value="1" {{ old('is_role') == '1' ? 'selected' : '' }}>Director General</option>
        <option value="2" {{ old('is_role') == '2' ? 'selected' : '' }}>Director Lands</option>
        <option value="3" {{ old('is_role') == '3' ? 'selected' : '' }}>Director Admin</option>
        <option value="4" {{ old('is_role') == '4' ? 'selected' : '' }}>Duty Officer</option>
        <option value="5" {{ old('is_role') == '5' ? 'selected' : '' }}>Duty Clerk</option>
        <option value="6" {{ old('is_role') == '6' ? 'selected' : '' }}>Duty Wo</option>
        <option value="7" {{ old('is_role') == '7' ? 'selected' : '' }}>Duty Driver</option>
        <option value="8" {{ old('is_role') == '8' ? 'selected' : '' }}>Duty Radio/Operator</option>
    </select>
